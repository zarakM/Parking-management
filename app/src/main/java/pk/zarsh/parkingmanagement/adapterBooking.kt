...
class adapterBooking(internal var mContext: Context, internal var mData: ArrayList<_BookingDetail?>) : RecyclerView.Adapter<adapterBooking.myViewHolder>() {
    ...
    override fun onBindViewHolder(holder: myViewHolder, position: Int) {
        ...
        holder.itemView.setOnLongClickListener {
            ...
            val tref = FirebaseDatabase.getInstance().getReference("Booking_details/Tariq")
            tref.orderByChild("id").equalTo(company?.id.toString()).addValueEventListener(object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    TODO("not implemented") //To change body of created functions use File | Settings | File Templates.
                }
            ...
            val gref = FirebaseDatabase.getInstance().getReference("Booking_details/Gulshan")
            gref.orderByChild("id").equalTo(company?.id.toString()).addValueEventListener(object : ValueEventListener {
                override fun onCancelled(p0: DatabaseError) {
                    TODO("not implemented") //To change body of created functions use File | Settings | File Templates.
                }
